    ...
    try:
        ...
        if operation == "INSERT":
            if table.startswith("Hub_"):
                process_hub(table, record)
            elif table.startswith("Link_"):
                process_link(table, record)
            elif table.startswith("Sat_"):
                process_satellite(table, record)
        elif operation == "UPDATE":
            # Handle updates if necessary
            handle_update(table, record)
        elif operation == "DELETE":
            # Handle deletes if necessary
            handle_delete(table, record)

    except Exception as e:
        print(f"Error processing message: {e}")

# Обработка вставки данных в Hub
def process_hub(table, record):
    hash_key = str(uuid.uuid5(uuid.NAMESPACE_DNS, record["Airport_Code"]))
    record["Airport_HK"] = hash_key

    insert_query = f"""
        INSERT INTO {table} (Airport_HK, Airport_Code, Load_DTS, Record_Source)
...
def process_link(table, record):
    link_key = str(uuid.uuid4())  # Генерируем уникальный ключ для линка
    record["Link_HK"] = link_key

    insert_query = f"""
        INSERT INTO {table} (Link_HK, Flight_HK, Departure_Airport_HK, Arrival_Airport_HK, Load_DTS, Record_Source)
...
def process_satellite(table, record):
    insert_query = f"""
        INSERT INTO {table} (Airport_HK, Airport_Name, City, Coordinates_Lon, Coordinates_Lat, Timezone, Load_DTS, Record_Source, Effective_From, Effective_To)
...
def handle_update(table, record):
    # Implement update logic if needed
    pass

# Функция для обработки удалений
def handle_delete(table, record):
