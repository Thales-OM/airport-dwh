SELECT 
    ...
    -- По заданию не очень понятно нужно ли считать чиcло уникальных людей (passenger_id) или просто пассажирова (включая кейсы когда один человек летит в одну сторону 2+ раза)
...
INNER JOIN 
    dwh_detailed.Hub_Airports a1 
        ON lfa.Arrival_Airport_HK = a1.Airport_HK
INNER JOIN 
    dwh_detailed.Hub_Airports a2 
        ON lfa.Departure_Airport_HK = a2.Airport_HK
INNER JOIN 
    dwh_detailed.Sat_Flights f 
        ...
        AND f.Actual_Departure is not NULL -- Не рассматриваем отмененные рейсы
LEFT JOIN 
    dwh_detailed.Link_Tickets_Flights ltf 
        ON f.Flight_HK = ltf.Flight_HK
GROUP BY 
    f.Actual_Departure, GREATEST(a1.Airport_Code, a2.Airport_Code), LEAST(a1.Airport_Code, a2.Airport_Code)
...
LEFT JOIN 
    dwh_detailed.Link_Tickets_Flights ltf 
        ON f.Flight_HK = ltf.Flight_HK
GROUP BY 
    f.Actual_Departure, LEAST(a1.Airport_Code, a2.Airport_Code), GREATEST(a1.Airport_Code, a2.Airport_Code)
;